...
def evaluate_policy(
    ...
    # --- NEW: store only Q-values and rewards ---
    traj = dict(q=[], rew=[])
    ...
    while (episode_counts < episode_count_targets).any():
        ...
        if collect_trajectories:
            with torch.no_grad():
                obs_t = torch.as_tensor(observations[0]).unsqueeze(0).float().to(model.device)
                act_t = torch.as_tensor(actions[0]).unsqueeze(0).float().to(model.device)
                if hasattr(model, "critic"):  # DDPG-like
                    q_val = model.critic(obs_t, act_t)[0]
                elif hasattr(model, "qf1"):  # TD3-like
                    q_val = model.qf1(obs_t, act_t)
                traj["q"].append(float(q_val.squeeze().cpu().numpy()))
                traj["rew"].append(float(rewards[0]))
    ...
    if return_episode_rewards:
        return episode_rewards, episode_lengths, traj
