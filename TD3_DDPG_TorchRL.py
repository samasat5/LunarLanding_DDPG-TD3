...
@torch.no_grad()
def eval_mc_bias(policy, critic, env, gamma=0.99, episodes=10, device="cpu"):
    """
    Estimates over/under-estimation by comparing critic Q(s,a) vs Monte-Carlo return G_t
    on rollouts generated by the *deterministic* policy (no exploration).
    Returns dict with mean/std and raw samples.
    """
    policy.eval()
    # critic is a TensorDictModule that takes {"observation","action"} -> {"state_action_value"}

    all_bias, all_q, all_g = [], [], []
    ...
    for _ in range(episodes):
        ...
        while True:
            # use the observation BEFORE stepping for (s_t, a_t)
            obs_t = td.get("observation")
            # deterministic action from current policy (no OUNoise here)
            out = policy(TensorDict({"observation": obs_t}, batch_size=obs_t.shape[:-1]))
            a_t = out.get("action")

            # record
            obs_list.append(obs_t.to(device))
            act_list.append(a_t.to(device))

            # step
            td = env.step(a_t)
            r_t = td.get(("next", "reward")).item()
            rew_list.append(r_t)

            if td.get(("next", "done")).item():
                break

        # compute return-to-go G_t (backward pass)
        R = 0.0
        G = []
        ...
        # critic estimates on same (s_t, a_t)
        obs_batch = torch.stack(obs_list)                     # [T, obs_dim]
        act_batch = torch.stack([a.squeeze(0) for a in act_list])  # [T, act_dim]

        td_q = TensorDict(
            {"observation": obs_batch, "action": act_batch},
            batch_size=[len(G_t)]
        )
        q_hat = critic(td_q).get("state_action_value").squeeze(-1)  # [T]

        # accumulate
        bias = (q_hat - G_t).cpu().numpy()
        all_bias.extend(bias)
        all_q.extend(q_hat.cpu().numpy())
        all_g.extend(G_t.cpu().numpy())

    import numpy as np
    return {
        "bias_mean": float(np.mean(all_bias)),
        "bias_std": float(np.std(all_bias)),
